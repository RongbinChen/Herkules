...
export default function ContractsPage() {
  ...
  const {
    unlock, team, setTeam, doUnlock, lock, busy, error, setError,
    configured, refreshPinStatus, handleAuthError,
        ...
        <div className="mx-auto max-w-lg">
          ...
          <Card className="p-6">
            ...
            <form onSubmit={submitPin} className="flex gap-2">
              <Input type="password" value={pin} onChange={(e) => setPin(e.target.value)} placeholder={`${team} PIN`} autoComplete="off" />
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-1.5">
          {['WRC', 'HRC'].map((k) => (
            <button
              key={k}
              onClick={() => { if (k !== unlock.team) { setTeam(k); lock() } }}
              className={`rounded-full px-3 py-1 text-xs font-semibold transition ${
