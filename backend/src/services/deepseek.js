...
- 粒子治疗/质子/重离子/放射治疗加速器——2026-08-09 起不再算作目标市场
- 半导体与显示面板产线的专用制程设备（刻蚀机、光刻胶剥离、溅射镀膜、CMP 与晶圆减薄磨床、AMOLED/TFT 产线设备等），哪怕名称里带"磨床"
- 与机床/磨床/重型精密机械无关的通用采购
注意：普通"医疗影像/诊断设备"不是本公司市场，必须过滤掉。
...
export async function callDeepSeek(messages, maxTokens = 200) {
  if (!API_KEY) {
    throw new DeepSeekError(deepseekFailureMessage(401), 401);
  }
  let res;
  try {
  ...
  if (!res.ok) {
    // Classified: 402 out-of-balance / 401 bad key / 429 rate limit / other.
...
const REPORT_SYSTEM_EN = `You are a China-market analyst serving Herkules Group and Waldrich Siegen (manufacturers of roll grinders and heavy machine tools).
Based on the given tender statistics, write a concise English market brief (Markdown), covering:
1. **Market Overview** — tender activity this period and key equipment-related signals
2. **Key Projects** — tenders worth sales follow-up (with deadlines)
3. **Competitor Moves** — competitor awards and what they imply
4. **Trend Read** — demand shifts by industry / region / equipment type, and recommendations for both companies' China business
Keep it professional, direct and management-facing, under 400 words.`;

/**
 * Generate a market brief from aggregated statistics.
 * `statsContext` is a plain-text/JSON dump of the aggregated data.
 * `lang` — 'zh' (default) or 'en' for the output language.
 */
export async function generateMarketReport(statsContext, lang = 'zh') {
  try {
    return await callDeepSeek([
      { role: 'system', content: lang === 'en' ? REPORT_SYSTEM_EN : REPORT_SYSTEM },
